...
var home = process.env.INSIGHT_DB || ('/work/www/explorer.shrooms.pw/shroomssight/.db');
...
if (!dataDir) {
  if (isWin) dataDir = '%APPDATA%\\SHROOMS\\';
  if (isMac) dataDir = process.env.HOME + '/Library/Application Support/SHROOMS/';
  if (isLinux) dataDir = process.env.HOME + '/.SHROOMS/';
}
dataDir += network === 'testnet' ? 'testnet' : '';

var safeConfirmations = process.env.INSIGHT_SAFE_CONFIRMATIONS || 10;
var ignoreCache = process.env.INSIGHT_IGNORE_CACHE || 0;


var bitcoindConf = {
  protocol: process.env.BITCOIND_PROTO || 'http',
  user: process.env.BITCOIND_USER || 'SHROOMSrpc',
