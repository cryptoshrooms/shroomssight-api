...
var genesisTXID = 'c18404b003fb8a9644fb68c73207d19ad250dc054314798ca91c52bb355f2c3f';
var CONCURRENCY = 10;
var DEFAULT_SAFE_CONFIRMATIONS = 10;
...
var bitcore = require('shroomscore'),
...
var encodedData = require('soop').load('shroomscore/util/EncodedData', {
  base58: base58
});
var versionedData = require('soop').load('shroomscore/util/VersionedData', {
  parent: encodedData
});

var Address = require('soop').load('shroomscore/lib/Address', {
  ...
  async.eachLimit(txInfo.vin, CONCURRENCY, function(i, c_in) {
      self.fromTxIdN(i.txid, i.vout, function(err, ret) {
        if (!ret || !ret.addr || !ret.valueSat) {
          ...
          incompleteInputs = 1;
          return c_in(); // error not escalated
        }

        txInfo.firstSeenTs = ret.ts;
        i.unconfirmedInput = ret.unconfirmedInput;
        i.addr = ret.addr;
        i.valueSat = ret.valueSat;
        i.value = ret.valueSat / util.COIN;
        valueIn += i.valueSat;

        if (!txInfo.isCoinStake && (ret.multipleSpentAttempts || !ret.spentTxId ||
...
TransactionDb.prototype.getStandardizedTx = function(tx, time) {
  var self = this;
...
TransactionDb.prototype._addScript = function(tx, relatedAddrs) {
  ...
  for (ii = 0; ii < tx.vout.length; ii++) {
    var o = tx.vout[ii];
    // TODO : not supported=> standard multisig
